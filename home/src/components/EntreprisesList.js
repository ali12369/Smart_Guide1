...
export default function EntreprisesList() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5001/adminstration/get_all_adminstrations"
      );
  ...
  const deleteItem = async (id) => {
    try {
      const response = await axios.delete(
        `http://localhost:5001/adminstration/delete_adminstration/${id}`
      );
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Main open={open}>
        <DrawerHeader />
        <Typography variant="h5">Entreprises List :</Typography>
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            <TableHead>
              <TableRow>
                <TableCell>Entreprises Name</TableCell>
